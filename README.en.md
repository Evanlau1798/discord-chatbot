...
The drawing protocol accepts only explicit `create` or `edit` operations. The model chooses the operation from a source-dependency rule: if the requested output depends on a source image, it must use `edit`. The harness does not classify natural-language intent with keywords; it only validates that `edit` uses trusted candidate IDs from the current request, with at most one constrained repair for unknown IDs. Recent images are not attached to the initial multimodal request. It exposes only traceable message reference IDs for the same user and channel, and the model may request one deferred image load when needed. Current attachments, reply chains, and explicit Discord message links remain direct trusted candidates; unavailable historical references never fall back to generation from scratch.
