...
class AiChatBrowserFlowMixin:
    async def _complete_after_browser(
        ...
        operation_kwargs = (
            {"image_operation_policy": image_operation_policy} if image_operation_policy is not None else {}
        )
        ...
        final, _ = await self._complete_and_parse_with_raw(
            final_messages,
            message,
            persona_key,
            request_status,
